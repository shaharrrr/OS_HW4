...
private:
    ...
    typedef struct MallocMetadata {
        ...
        bool is_huge;   //Flag indicating if the block is a huge page
        ...
        MallocMetadata()
                : size(0), is_free(false),is_huge(false), next(nullptr), prev(nullptr) {}
    } *mallocMetadata;
...
public:
     ...
     */
    void* lalloc_block(size_t size, bool huge_page_flag = false);
...
bool MemoryBlocksManager::join(mallocMetadata* free_block){
    if((*free_block)->size > (BLOCKS_SET_SIZE*KB/2)-sizeof(struct MallocMetadata)) {   //Can't join blocks of maximal size
        return false;
    }
...
bool MemoryBlocksManager::can_join(mallocMetadata free_block, size_t size, size_t dest_size){
    if(dest_size > (BLOCKS_SET_SIZE*KB - sizeof(struct MallocMetadata))) {  //dest_size exceeds maximum manageable block size
        return false;
    }
    if(size-sizeof(struct MallocMetadata)  >= dest_size) {
        return true;
    }
    //Get the potential buddy using the XOR trick provided by staff
 ...
 */
void* MemoryBlocksManager::lalloc_block(size_t size, bool huge_page_flag) {
    // mmap() parameters:
    ...
    // - flags: MAP_PRIVATE | MAP_ANONYMOUS (if huge_page-flag is on, also add MAP_HUGETLB flag)
    ...
    MallocMetadata* block = (MallocMetadata*)mmap(nullptr, size + sizeof(MallocMetadata), PROT_READ | PROT_WRITE, flags, -1, 0);

    block->size = size;
    block->is_huge = huge_page_flag;
    block->is_free = false;
    block->next = nullptr;
    block->prev = nullptr;
...
void MemoryBlocksManager::free_block(void *p) {
    ...
    if(isLargeAlloc(block_metadata->size)){     //if block is a lerge allocation, use unmap with size alligned as a multiple of PAGESIZE
        ...
        int pageSize = block_metadata->is_huge ? 2*KB*KB : sysconf(_SC_PAGESIZE);
        if(size_allocated%pageSize != 0) {
            size_allocated += pageSize - size_allocated % pageSize;
        }
        if(munmap((void*)block_metadata, size_allocated) == -1){
            size_allocated += KB*KB*KB - size_allocated % KB*KB*KB;
...
void* smalloc(size_t size) {
    ...
    if (MemoryBlocksManager::getInstance().isLargeAlloc(size)) {
        return MemoryBlocksManager::getInstance().lalloc_block(size, size >= 4 * KB * KB); // Large allocation
    }
...
void* scalloc(size_t num, size_t size) {
    ...
    void* result;
    if(MemoryBlocksManager::getInstance().isLargeAlloc(size)) {
        result = MemoryBlocksManager::getInstance().lalloc_block(size * num, size >= 4 * KB * KB);
    }
    else {
        result = MemoryBlocksManager::getInstance().alloc_block(size * num);
    }
    if (!result) {
...
void* srealloc(void* oldp, size_t size){
    ...
    if(MemoryBlocksManager::getInstance().isLargeAlloc(size)){
        ...
        res = MemoryBlocksManager::getInstance().lalloc_block(size, size >= 4*KB*KB);
    }
    else {
